            ...
            <template v-else-if="activeFilter === 'circles'">
              ...
              >
                <div class="circle-result-header">
                  <img v-if="item.userAvatar" :src="item.userAvatar" class="user-avatar-sm" @error="(e) => e.target.src = defaultAvatar" />
...
const doSearch = async () => {
  ...
  try {
    if (activeFilter.value === 'users') {
      ...
      results.value = (pageData.records || []).map(u => ({ ...u, type: 'user' }))
      ...
      if (circlesRes.status === 'fulfilled' && circlesRes.value.data) {
        const circles = circlesRes.value.data || []
        results.value.push(...circles.map(c => ({ ...c, type: 'circle' })))
