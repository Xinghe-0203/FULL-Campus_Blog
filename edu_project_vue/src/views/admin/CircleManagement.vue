...
const typeText = (type) => {
  const map = { text: '文字', image: '图片', video: '视频', repost: '转发' }
  return map[type] || '文字'
}

const visibilityText = (visibility) => {
  const map = { public: '公开', followers: '仅关注', private: '私密' }
  return map[visibility] || '公开'
}

const visibilityClass = (visibility) => {
  const map = { public: 'visibility-public', followers: 'visibility-followers', private: 'visibility-private' }
