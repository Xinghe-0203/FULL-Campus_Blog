<template>
  <div class="admin-posts">
    ...
    <Modal :show="showRejectDialog" title="驳回原因" @close="cancelReject">
      <textarea
      ...
      <template #footer>
        ...
        <button class="btn btn-primary" style="margin-left: 8px;" @click="confirmReject">确认驳回</button>
      </template>
    </Modal>
    <ConfirmDialog />
  </div>
...
const getStatusClass = (status: number) => {
  const classes: Record<number, string> = { 0: 'status-pending', 1: 'status-active', 2: 'status-rejected' }
  return classes[status] || ''
}

const getStatusDotClass = (status: number) => {
  const classes: Record<number, string> = { 0: 'pending', 1: 'active', 2: 'rejected' }
  return classes[status] || ''
}

const getStatusText = (status: number) => {
  const texts: Record<number, string> = { 0: '待审核', 1: '已发布', 2: '已驳回' }
...
let rejectResolve: ((value: any) => void) | null = null
...
function confirmReject() {
  ...
  rejectResolve?.(rejectReason.value)
  rejectResolve = null
}
